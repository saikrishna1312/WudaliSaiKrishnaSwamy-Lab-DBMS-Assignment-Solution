...
/*
-- Display the number of the customer group by their genders who have
...
select C.Cus_Gender, count(C.Cus_Gender) as count from customer as C
inner join `order` on c.cus_id = `order`.cus_id where `order`.ord_amount >= 3000 group by C.Cus_Gender;

/* -- Display all the order along with product name ordered by a customer having Customer_Id=2; */
...
select supplier.supp_id, supplier.supp_name, customer.cus_name, rating.rat_ratstars from rating
...
/*-- Create a stored procedure to display the Rating for a Supplier if any along with the Verdict on that rating if any like if rating >4
...
END AS verdict from rating inner join supplier on supplier.supp_id=rating.supp.id;
END
